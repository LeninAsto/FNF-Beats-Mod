...
// MODIFICATION: New boolean to use the sprite color
uniform bool useSpriteColor;
...
vec3 createDropShadow(vec3 col, float curThreshold, bool useMask) {
  ...
  // MODIFICATION: We decided what color to use for the light.
  // If useSpriteColor is true, it uses the current pixel color (col), otherwise it uses dropColor.
  vec3 finalLightColor = useSpriteColor ? col : dropColor.rgb;

  // We apply the selected color
  col.rgb += finalLightColor * ((1.0 - (dropShadowAmount * str)) * intensity);
